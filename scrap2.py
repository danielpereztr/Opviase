...
for url in urls_seleccionadas:
    ...
    for row in capturas:
        ...
        # Descargamos la página archivada para extraer el precio medio (si existe) y el número de casas/pisos
        try:
        ...
        # Extraer número de casas/pisos del h1 (por ejemplo, "2 casas y pisos en alquiler...")
        cabecera_h1 = soup.find('h1', id='h1-container')
        if cabecera_h1:
            texto_h1 = cabecera_h1.get_text(strip=True)
            # Buscamos un número antes de "casas y pisos"
            match_num_casas_pisos = re.search(r'(\d+)\s+casas y pisos', texto_h1)
            if match_num_casas_pisos:
                num_casas_pisos = match_num_casas_pisos.group(1)
            else:
                num_casas_pisos = None
        else:
            num_casas_pisos = None
        ...
        nuevos_resultados.append({
            ...
            'barrio': barrio,
            'precio_medio': precio,
            'num_casas_pisos': num_casas_pisos  # Nuevo campo
        })
...
with open(CSV_FILENAME, 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(
        ...
        fieldnames=[
            ...
            'barrio',
            'precio_medio',
            'num_casas_pisos'  # Incluimos aquí también el nuevo campo
        ]
    )
    writer.writeheader()
